  ...
  return KB.projects
    .map((p) => {
      ...
      if (p.appStore) {
        lines.push(
          bullet(
            `On the iOS App Store${p.appStoreName ? ` as "${p.appStoreName}"` : ''}. ` +
              `Direct link: ${p.appStore}. Give this link when asked where to download it; ` +
              `do not send anyone off to search for it.`
          )
        );
      }
